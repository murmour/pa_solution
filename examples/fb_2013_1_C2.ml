...
Solution (w, h, p, q, n, x, y, a, b, c, d: int) : int64 =
  ...
  let process_events map ev =
    let map =
      List.fold_left (fun map -> function
        | `Add (y1, y2) -> add_range y1 y2 map
        | `Rem (y1, y2) -> rem_range y1 y2 map) map ev
    in
    sum := Int64.add !sum (Int64.of_int (calc_range map));
    map
  in

  let map = { l = 0; r = h; contents = Fill 0 } in
  ignore (events |> Array.fold_left process_events map);
  Int64.(sub (mul (of_int w) (of_int h)) !sum)
